...
class ChessGame {
  ...
  void start()
  {
    while (true) {
      ...
      printBoard();
      printTime();
      cout << "Current player: " << (currentPlayer == 'W' ? "White" : "Black")
           << endl;
      cout << "Enter move (e.g., e2e4), 'save' to save the game, or 'exit' to "
              "quit: ";
      string input;
      cin >> input;
      ...
      if (isValidMove(input)) {
        ...
        switchPlayer();
        if (isCheckmate()) {
          cout << (currentPlayer == 'W' ? "Black" : "White")
  ...
  void initializeBoard()
  {
    ...
    pieceNames = { { 'r', "Rook" }, { 'n', "Knight" }, { 'b', "Bishop" },
      { 'q', "Queen" }, { 'k', "King" }, { 'p', "Pawn" }, { 'R', "Rook" },
  ...
  void updateClock(const steady_clock::time_point& start,
