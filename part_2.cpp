bool isValidPawnMove(char piece, int fromX, int fromY, int toX, int toY)
  {
    ...
    if (fromY == toY) {
      ...
      if (fromX == startRow && fromX + 2 * direction == toX
          && board[fromX + direction][toY] == ' ' && board[toX][toY] == ' ') {
        return true;
      }
    } else if (abs(fromY - toY) == 1 && fromX + direction == toX
        && board[toX][toY] != ' ') {
      return true;
    }
  ...
  bool isValidBishopMove(int fromX, int fromY, int toX, int toY)
  {
    ...
    for (int x = fromX + stepX, y = fromY + stepY; x != toX;
         x += stepX, y += stepY) {
      if (board[x][y] != ' ') {
        return false;
      }
    ...
    if (destinationPiece != ' '
        && ((isupper(destinationPiece) && isupper(board[fromX][fromY]))
            || (islower(destinationPiece) && islower(board[fromX][fromY])))) {
      return false;
    }
  ...
  bool isKingInCheck(char player)
  {
    ...
    for (int x = 0; x < 8; ++x) {
      for (int y = 0; y < 8; ++y) {
        ...
        if ((opponent == 'W' && isupper(piece))
            || (opponent == 'B' && islower(piece))) {
  ...
  void makeMove(const string& move)
  {
    ...
    if ((toX == 0 && board[toX][toY] == 'P')
        || (toX == 7 && board[toX][toY] == 'p')) {
      promotePawn(toX, toY);
    }
  }
  ...
  void promotePawn(int x, int y)
  {
    ...
    while (true) {
      ...
      if (newPiece == 'Q' || newPiece == 'R' || newPiece == 'B'
          || newPiece == 'N') {
